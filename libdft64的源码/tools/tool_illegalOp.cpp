...
#define LOGFILE_DFL	"tool_illegalOp.log"
 ...
 */
static void PIN_FAST_ANALYSIS_CALL alert(ADDRINT ins, ADDRINT bt)
{
	printf("Illegal Operation Detected!\n");
	/* log file */
	FILE *logfile;
...
static void post_open_hook(THREADID tid, syscall_ctx_t *ctx)
{
	...
	/* add the fd of the document to the fdset */
	if (strstr((char *)ctx->arg[SYSCALL_ARG0], "shellcode.txt") != NULL)
	{
		socketset.insert((int)ctx->ret);
		//printf("insert shellcode.txt\n");
	}
}
...
static void post_openat_hook(THREADID tid, syscall_ctx_t *ctx) 
{
  /* sanity check */
	...
	/* add the fd of the document to the fdset */
	if (strstr((char *)ctx->arg[SYSCALL_ARG1], "shellcode.txt") != NULL)
	{
		socketset.insert((int)ctx->ret);
		//printf("insert shellcode.txt\n");
	}
