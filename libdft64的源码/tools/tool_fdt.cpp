...
/* default log file */
#define LOGFILE_DFL	"tool_fdt.log"

/* system sensitive file path */
#define PASSWD_PATH	"/etc/passwd"
#define SHADOW_PATH	"/etc/shadow"

/* server program sensitive file path(prefix) */
#define SSH_PATH	"/etc/ssh/"
/* #define THTTPD_PATH   "/etc/thttpd/" */
/* #define NGINX_PATH    "/etc/nginx/" */

/* sensitive file tag(for different sensitive file)*/
#define	TAG_PASSWD	0x01U
#define	TAG_SHADOW	0x02U
#define	TAG_SSH	    0x03U
/* #define TAG_THTTPD   0x04U */
/* #define TAG_NGINX    0x05U */
	...
	if (likely((logfile = fopen(logpath.Value().c_str(), "a")) != NULL)) {
		...
		switch(tag)
		{
		   case TAG_PASSWD:
		       ...
		       (void)fprintf(logfile, "/etc/passwd :");
			   break;
		   
		   case TAG_SHADOW:
		   
		       (void)fprintf(logfile, "/etc/shadow :");
			   break;
		   
		   case TAG_SSH:
		   
		       (void)fprintf(logfile, "/etc/ssh/ :");
			   break;
		   
		   default:
		   
		       (void)fprintf(logfile, "mixed :");
			   break;
...
static void post_open_hook(THREADID tid, syscall_ctx_t *ctx)
{
	...
	/* add the fd of the document to the fdset */
	if (strstr((char *)ctx->arg[SYSCALL_ARG0], PASSWD_PATH) != NULL ||
		strstr((char *)ctx->arg[SYSCALL_ARG0], SHADOW_PATH) != NULL ||
		strstr((char *)ctx->arg[SYSCALL_ARG0], SSH_PATH) != NULL)
...
static void post_openat_hook(THREADID tid, syscall_ctx_t *ctx) 
{
  /* sanity check */
	...
	/* add the fd of the document to the fdset */
	if (strstr((char *)ctx->arg[SYSCALL_ARG1], PASSWD_PATH) != NULL ||
		strstr((char *)ctx->arg[SYSCALL_ARG1], SHADOW_PATH) != NULL ||
		strstr((char *)ctx->arg[SYSCALL_ARG1], SSH_PATH) != NULL)
		fdset.insert((int)ctx->ret);
}
...
static void post_read_hook(THREADID tid, syscall_ctx_t *ctx)
{
	...
	if (fdset.find((int)ctx->arg[SYSCALL_ARG0]) != fdset.end())
	{
			...
			if(strstr(file_path, PASSWD_PATH) != NULL)
			      /* set the tag markings */
	              tagmap_setn(ctx->arg[SYSCALL_ARG1], (size_t)ctx->ret, TAG_PASSWD);
		    else if(strstr(file_path, SHADOW_PATH) != NULL)
			      /* set the tag markings */
	              tagmap_setn(ctx->arg[SYSCALL_ARG1], (size_t)ctx->ret, TAG_SHADOW);
			else
			      /* set the tag markings */
	              tagmap_setn(ctx->arg[SYSCALL_ARG1], (size_t)ctx->ret, TAG_SSH);		
	}
	else
	...
	/* auditing */
	logfile = fopen(logpath.Value().c_str(), "a");
	(void)fprintf(logfile, "[%d]: buff:0x%08lx , buffsize:%ld\n",
							getpid(), ctx->arg[SYSCALL_ARG1], ctx->ret);
}
...
static void post_readv_hook(THREADID tid, syscall_ctx_t *ctx)
{
	...
	for (i = 0; i < (int)ctx->arg[SYSCALL_ARG2] && tot > 0; i++) 
	{
		...
		if (it != fdset.end())
		{
			      ...
			      if(strstr(file_path, PASSWD_PATH) != NULL)
			            /* set the tag markings */
	                    tagmap_setn((size_t)iov->iov_base, iov_tot, TAG_PASSWD);
		          else if(strstr(file_path, SHADOW_PATH) != NULL)
			            /* set the tag markings */
	                    tagmap_setn((size_t)iov->iov_base, iov_tot, TAG_SHADOW);
			      else
			            /* set the tag markings */
	                    tagmap_setn((size_t)iov->iov_base, iov_tot, TAG_SSH);		
	    }
		else
                	/* clear the tag markings */
